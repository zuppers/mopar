...
public class BufferedFile {
    ...
    public BufferedFile(Path path, String access, int bufferLen) throws IOException {
        this(path, access, bufferLen, bufferLen);
    }

    public BufferedFile(Path path, String access, int writeBufferLen, int readBufferLen) throws IOException {
        this(new FileOnDisk(path, access), writeBufferLen, readBufferLen);
    }
    ...
    public void write(byte[] src, int off, int len) throws IOException {
        try {
            ...
            // If we are over writing a write buffered section of the file then flush the buffer before we continue
            if (writePosition != -1L && (position < writePosition || position > (long) writeBufferLen + writePosition)) {
                flushWriteBuffer();
            }
            ...
            if (writePosition != -1L && (long) writeBuffer.length + writePosition < position + (long) len) {
                ...
                len -= i;
                System.arraycopy(src, off, writeBuffer, (int) (position - writePosition), i);
                position += (long) i;
            ...
            if (writeBuffer.length < len) {
                ...
                long upperPos = -1L;
                if (readPosition > (long) len + position && (long) len + position <= (long) readBufferLen + readPosition)
                    upperPos = (long) len + position;
                else if (position < readPosition + (long) readBufferLen && readPosition + (long) readBufferLen <= (long) len + position) {
                    upperPos = readPosition + (long) readBufferLen;
                }

                if (lowerPos > -1L && upperPos > lowerPos) {
                    int i = (int) (upperPos - lowerPos);
                    System.arraycopy(src, (int) (-position + ((long) off + lowerPos)), readBuffer, (int) (lowerPos + -readPosition), i);
                }
                position += (long) len;
            } else if (len > 0) {
                if (writePosition == -1L) {
    ...
    public void read(byte[] dest) throws IOException, ArrayIndexOutOfBoundsException {
        read(dest, 0, dest.length);
    }

    public void read(byte[] dest, int off, int len) throws IOException, ArrayIndexOutOfBoundsException {
        ...
        try {
            ...
            if (writePosition != -1L && position >= writePosition && (long) writeBufferLen + writePosition >= position + (long) len) {
            ...
            if (writePosition != -1L) {
                ...
                // write occurred, this is under the assumeption we reached the
                // end of the file
                ...
                long upperPos = -1L;
                if (pos < (long) writeBufferLen + writePosition && writePosition + (long) writeBufferLen <= (long) readLen + pos) {
                    upperPos = writePosition + (long) writeBufferLen;
                } else if (((pos + (long) readLen) > (writePosition)) && pos + (long) readLen >= (long) writeBufferLen + writePosition) {
                ...
                if (lowerPos > -1L && upperPos > lowerPos) {
                    ...
                    System.arraycopy(writeBuffer, (int) (lowerPos - writePosition), dest, (int) (lowerPos - pos) + readOff, diff);
    ...
    public void flushWriteBuffer() throws IOException {
        if(writePosition != -1L) {
            ...
            } else if (readPosition >= writePosition && writePosition + (long) writeBufferLen > readPosition) {
                upperPos = readPosition;
            }

            long lowerPos = -1L;
            if (writePosition + (long) writeBufferLen <= readPosition || readPosition + (long) readBufferLen < (long) writeBufferLen + writePosition) {
                if (writePosition < (long) readBufferLen + readPosition && (long) readBufferLen + readPosition <= (long) writeBufferLen + writePosition) {
            ...
            if (upperPos > -1L && upperPos < lowerPos) {
                ...
                System.arraycopy(writeBuffer, (int) (upperPos - writePosition), readBuffer, (int) (upperPos - readPosition), len);
            }
            writePosition = -1L;
            writeBufferLen = 0;
        }
    ...
    public void fillReadBuffer() throws IOException {
        readBufferLen = 0;
